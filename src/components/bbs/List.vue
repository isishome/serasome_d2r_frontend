<script setup>
import { defineAsyncComponent, reactive, ref, shallowReadonly, watch, inject } from 'vue'
...
const parsThumbnail = (thumbnail) => {
  return decodeURIComponent(thumbnail) === 'null' ? new URL('/images/blank.png', import.meta.url).href : decodeURIComponent(thumbnail)
}
    ...
    <Table ref="table" :pagination.sync="pagination" :columns="columns[sec] || columns.default" :data="data"
        ...
        <q-tr class="cursor-pointer text-center" @click="rowClick(props.row.pid)"
          ...
          <q-td>
            <q-img class="rounded-borders" :ratio="20 / 13" style="width:80px" :src="parsThumbnail(props.row.thumbnail)"
              no-spinner>
              <template #error>
                <q-img no-spinner :ratio="2 / 1" src="/images/blank.png" class="absolute-center" />
              </template>
            </q-img>
          </q-td>
          ...
          <q-td>
            <div class="row items-center q-gutter-x-xs">
              <q-avatar rounded class="q-mr-xs avatar" size="30px">
                <q-img v-if="props.row.avatar" :src="props.row.avatar" :ratio="1" no-spinner>
                  <template #error>
        ...
        <div class="q-pa-sm col-xs-6 col-sm-4 col-md-3">
          <q-card @click="rowClick(props.row.pid)" class="d2r-card-list"
            ...
            <q-img :src="parsThumbnail(props.row.thumbnail)" ratio="1"
              :style="$q.screen.lt.sm ? 'height:80px' : 'height:100px'" no-spinner>
              <template #error>
                <q-img no-spinner :ratio="2 / 1" src="/images/blank.png" class="absolute-center" />
              </template>
            </q-img>
      ...
      <template #pagination>
        <q-pagination v-if="data.length > 0" class="pagination row justify-end"
          ...
          color="invert" @update:model-value="go" :max="pagination.rowsNumber" :max-pages="5" :ellipses="false"
