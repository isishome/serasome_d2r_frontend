...
const sunder = reactive({})
const data = reactive([])

mergeSection('items', 'sunder').then(() => {
    Object.assign(sunder, tm('sunder'))
    data.push(...tm('sunderData'))
})
</script>
...
<template>
    <div>
        <p v-for="(d, idx) in sunder.desc" :key="idx">
            {{ d }}</p>
        <q-table class="no-shadow knowledge-table" card-container-class="q-col-gutter-md justify-start"
            :grid="screen.lt.lg" :rows="data" :columns="columns" row-key="id" :pagination="pagination" dense bordered
            hide-header hide-pagination wrap-cells>
            <template #body="props">
